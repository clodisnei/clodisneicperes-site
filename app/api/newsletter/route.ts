import { joinNewsletter } from "@/lib/site-metrics";
...
export async function POST(request: Request) {
  try {
    ...
    if (!emailPattern.test(email) || email.length > 254) return Response.json({ error: "Digite um e-mail válido." }, { status: 400 });
    if (payload.consent !== true) return Response.json({ error: "Confirme que deseja receber novidades." }, { status: 400 });
